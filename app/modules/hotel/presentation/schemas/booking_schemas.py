...
class CreateBookingRequestSerializer(serializers.Serializer):
    ...
    guest_count      = serializers.IntegerField(min_value=1, max_value=20, required=False, default=0)
    adults           = serializers.IntegerField(min_value=0, default=1, required=False)
    ...
    def validate(self, data):
        ...
        # Si guest_count n'est pas fourni, le calculer depuis adults + children
        if not data.get("guest_count"):
            data["guest_count"] = data.get("adults", 1) + data.get("children", 0)
        if data["guest_count"] < 1:
            raise serializers.ValidationError(
                {"guest_count": "Le nombre de voyageurs doit être au moins 1."}
            )
        return data
